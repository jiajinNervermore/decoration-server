...
router.post('/addMessage', (req, res, next) => {
  ...
  let sql = 'insert into dec_message(fromuid,uname,avatar,touid,mes,fTime,files) values(?,?,?,?,?,?,?)';
  try {
    query(sql, [fromuid,uname,avatar, touid, mes, fTime, files]).then(result => {
      let output = {
        code: 200,
        msg: 'add succ',
...
router.get('/messagelist', (req, res, next) => {
  let sql = 'select * from dec_message';
  ...
  try {
    (async function () {
      ...
      if (newUserList.length > 0) {
        ...
        for (var i = 0; i < len; i++) {
          (function (sql2, i, len) {
            query(sql2, [newUserList[i].fromuid]).then(
              result => {
